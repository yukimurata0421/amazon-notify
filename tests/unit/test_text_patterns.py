...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            '"Amazon.co.jp" <shipment-tracking@amazon.co.jp>',
            "shipment-tracking@amazon.co.jp",
        ),
        (
            "=?UTF-8?B?QW1hem9uLmNvLmpw?= <auto-confirm@amazon.co.jp>",
            "auto-confirm@amazon.co.jp",
        ),
        ("Amazon Notifications <order+test@amazon.co.jp>", "order+test@amazon.co.jp"),
    ],
)
def test_extract_email_address_handles_multiple_formats(
    from_header: str, expected: str
) -> None:
...
def test_is_amazon_mail_regression_cases(
    from_header: str, pattern: str, expected: bool
) -> None:
