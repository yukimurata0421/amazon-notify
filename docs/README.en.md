# Documentation Index

This is the primary navigation page for docs. Start with `HYBRID_QUICKSTART` and `OPERATIONS` for day-to-day usage.

## Setup
- [HYBRID_QUICKSTART.en.md](./HYBRID_QUICKSTART.en.md)
- [DOCKER.en.md](./DOCKER.en.md)

## Operations
- [OPERATIONS.en.md](./OPERATIONS.en.md)
- [HYBRID_ARCHITECTURE.en.md](./HYBRID_ARCHITECTURE.en.md)

## Design Notes
- [IMPLEMENTATION_RATIONALE.en.md](./IMPLEMENTATION_RATIONALE.en.md)
- [engineering-decisions.en.md](./engineering-decisions.en.md)
- [PORTABILITY_PARAMS.en.md](./PORTABILITY_PARAMS.en.md)

## 日本語
- [README.md](./README.md)
